...
var Worker = exports.Worker = function(options) {
    ...
    options = options || {};
    workerOptions = { withUnique: options.withUnique };
    if (options.hasOwnProperty('withUnique')) { delete options.withUnique; }

    // VALIDATION
    returned = common.verifyAndSanitizeOptions(workerOptions, { withUnique: false });
    if (returned instanceof Error) { return returned; }
    returned = common.verifyOptions(workerOptions, { withUnique: [true, false] });
    if (returned instanceof Error) { return returned; }
...
Worker.prototype.addFunction = function(name, callback, options) {
    ...
    for (var i = 0; i < this.jobServers.length; i ++) { // TODO iterate only servers without any previous error
        ...
        if (options.hasOwnProperty('timeout')) {
            jobServer.send(protocol.encodePacket(protocol.PACKET_TYPES.CAN_DO_TIMEOUT, 'ascii', [name, options['timeout']]));
        } else {
            jobServer.send(protocol.encodePacket(protocol.PACKET_TYPES.CAN_DO, 'ascii', [name]));
        }
...
Worker.prototype.removeFunction = function(name) {
    ...
    for (var i = 0; i < this.jobServers.length; i ++) { // TODO iterate only servers without any previous error
        jobServer = this.jobServers[i];
        jobServer.send(protocol.encodePacket(protocol.PACKET_TYPES.CANT_DO, 'ascii', [name]));
...
Worker.prototype.setWorkerId = function(workerId) {
    ...
    for (var i = 0; i < this.jobServers.length; i ++) { // TODO iterate only servers without any previous error
        jobServer = this.jobServers[i];
        jobServer.send(protocol.encodePacket(protocol.PACKET_TYPES.SET_CLIENT_ID, 'ascii', [this.workerId]));
    }
...
common.mixin({
    ...
    reportStatus: function (numerator, denominator) {
        ...
        var jobServer = this.clientOrWorker._getJobServerByUid(this.jobServerUid);
        jobServer.send(protocol.encodePacket(protocol.PACKET_TYPES.WORK_STATUS, null, [this.handle, numerator, denominator]));
    },
    /**
    ...
    reportWarning: function (data) {
        ...
        jobServer.send(protocol.encodePacket(protocol.PACKET_TYPES.WORK_WARNING, null, [this.handle, data.toString()]));
    },
    ...
    _sendAndClose: function (packetType, packetData) {
        var jobServer = this.clientOrWorker._getJobServerByUid(this.jobServerUid);
        jobServer.send(protocol.encodePacket(packetType, 'utf8', packetData));
